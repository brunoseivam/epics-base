...
static void once(void)
{
    ...
    status = osdPosixMutexInit(&onceLock,PTHREAD_MUTEX_NORMAL);
    checkStatusQuit(status,"osdPosixMutexInit","epicsThreadInit");
    status = osdPosixMutexInit(&listLock,PTHREAD_MUTEX_NORMAL);
    checkStatusQuit(status,"osdPosixMutexInit","epicsThreadInit");
